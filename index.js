...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5511952063035:+55 (11) 95206 3035\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oii @${num.split('@')[0]}\nBem vindo ao grupo, por favor, n�o seja ghost*${mdata.subject}*`
				...
				teks = `Foi tardeKKKKKK @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Calma ae porra, j�j� saiKKKKK ⌛',
				success: '✔️ Sucesso ✔️',
				...
				only: {
					...
					ownerB: '[❗] Este comando só pode ser usado pelo Lzinn! ❌',
			...
			const ownerNumber = ["5511952063035@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5511952063035@s.whatsapp.net'
                        /******ApiKey Input******/
